...
import com.AG_AP.electroshop.uiState.ListPurchaseOrderUiState
import com.AG_AP.electroshop.viewModels.ListPurchaseOrderViewModel
import com.AG_AP.electroshop.viewModels.Routes

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ListPurchaseOrderView(
    ...
    Row(
        ...
        Column(
            ...
            ListPurchaseOrderColumn(viewModel, dataUiState, navController)
        }
    }
}
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ScaffoldListPurchaseOrder(
    viewModel: ListPurchaseOrderViewModel = viewModel(),
    navController: NavHostController
) {
    ...
    Scaffold(
        ...
        Box(modifier = Modifier.padding(innerPadding)) {
            ListPurchaseOrderView(innerPadding, viewModel, navController)
        }
    }
}
...
@Composable
fun ListPurchaseOrderViewPreview() {
    //ListActivityViewPre()
}

@Composable
fun ListPurchaseOrderColumn(
    viewModel: ListPurchaseOrderViewModel,
    dataUiState: ListPurchaseOrderUiState,
    navController: NavHostController
) {
    Text(
...
@Composable
fun LazyColumnPurchaseOrder(data: List<OrderFireBase?>, navController: NavHostController) {
    LazyColumn {
        items(data) { item ->
            // Aquí defines cómo se muestra cada elemento de la lista
            ElevatedCardPurchaseOrder(item, navController)
        }
    }
}

@Composable
fun ElevatedCardPurchaseOrder(x: OrderFireBase?, navController: NavHostController) {
    ElevatedCard(
        ...
        Row(
            ...
            if (x != null) {
                Column {
                    Text(
                        text = x.DocNum.toString(),
                        modifier = Modifier
                            .padding(16.dp),
                        textAlign = TextAlign.Center,
                    )
                }
                Button(onClick = { navController.navigate(route = Routes.PurchaseOrderAux.route + "/${x.DocNum}") }) {
                    Text(text = "Ver")
                }
            }
        }
